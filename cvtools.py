...
def ipynb_show_color_histogram(histogram, plot_title=''):
    ...
    ax.bar(list(range(len(histogram))), histogram, color='r', width=1)
...
class VideoReader(object):
    ...
    def open(self, video_file_path):
        ...
        if not os.path.isfile(self.video_file_path):
            self._clear()
            raise IOError(
                'cannot open video: <%s> is not a valid file' % video_file_path)

        self.capture = cv2.VideoCapture(self.video_file_path)
        if not self.is_opened():
            self._clear()
            raise IOError(
                'cannot open video: <%s> is not a valid video file' % video_file_path)
